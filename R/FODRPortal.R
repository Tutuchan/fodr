...
FODRPortal <- R6::R6Class("FODRPortal",
                            public = list(
                              ...
                              initialize = function(portal){
                                self$portal <- portal
                                self$n_datasets <- get_datasets(portal = self$portal)$data$nhits
                              ...
                              search = function(nrows = NULL, refine = NULL, exclude = NULL, sort = NULL, q = NULL, lang = NULL, theme = NULL) {
                                ...
                                listDatasets <- get_datasets(portal = self$portal, nrows, refine, exclude, sort, q, lang)$data$datasets
                            ...
                            private = list(
                              get_themes = function(){
                                lapply(get_datasets(portal = self$portal, nrows = self$n_datasets)$data$datasets, function(dataset) {
                                  dataset$metas$theme
                                }) %>%
                                  unlist %>%
                                  unique %>%
